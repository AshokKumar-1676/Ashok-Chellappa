...
import { Home, Info, Map as MapIcon, Gift, Mail } from 'lucide-react';
...
const BottomNav: React.FC = () => {
  ...
  const navItems = [
    ...
    { id: 'contact', label: 'Contact', icon: Mail, path: AppRoute.CONTACT },
  ];
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center p-1 transition-all duration-300 min-w-[50px] ${
              ...
              >
                ...
                <span className={`text-[9px] font-extrabold mt-1 tracking-wide uppercase ${isActive ? 'opacity-100' : 'opacity-70'}`}>
                  {item.label}
                </span>
              </button>
            );
